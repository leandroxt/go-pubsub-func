...
func (app application) sendEvent(w http.ResponseWriter, r *http.Request) {
	...
	encodedData := base64.StdEncoding.EncodeToString(eventDataBytes)

	// Create the message wrapper
	message := MessageWrapper{
		Subscription: "projects/demo-test/subscriptions/my-subscription",
		Message: struct {
	...
	if err != nil {
		app.logger.Error("error encoding message wrapper", slog.String("error", err.Error()))
		w.WriteHeader(http.StatusInternalServerError)
		w.Write([]byte("something went wrong"))
		return
	}
